...
static RegisterPass<BranchProbabilities> X("SeansBranchProbabilities",
"Algorithm 1 from Static Brnch Frequency and Program Profile Analysis", false, false);
...
void BranchProbabilities::PredictAsNotTaken(int i, float fProb, const char *s)
{
  int iOther = (i * -1) + 1;
  PredictAsTaken(iOther, fProb, s);
}

void BranchProbabilities::PredictAsTaken(int i, float fProbH, const char *s)
{
  if ((i != 0) && (i != 1))
    return;
